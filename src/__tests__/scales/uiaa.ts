...
describe('UIAA', () => {
    ...
    describe('Invalid grade format', () => {
      ...
      test('invalid plus modifier', () => {
        ...
        expect(console.warn).toHaveBeenCalledWith('Unexpected grade format: +5 for grade scale UIAA')
      ...
      test('extra slash grade', () => {
        ...
        expect(console.warn).toHaveBeenCalledWith('Unexpected grade format: 5/5+/6- for grade scale UIAA')
        expect(invalidGrade).toEqual(-1)
      })
      test('not UIAA scale', () => {
        const invalidGrade = UIAA.getScore('V11')
        expect(console.warn).toHaveBeenCalledWith('Unexpected grade format: V11 for grade scale UIAA')
        expect(invalidGrade).toEqual(-1)
      })
    })
